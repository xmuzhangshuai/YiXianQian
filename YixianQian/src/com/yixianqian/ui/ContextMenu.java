...
public class ContextMenu extends BaseActivity {
	...
	public void copy(View view) {
		setResult(ChatActivity2.RESULT_CODE_COPY, new Intent().putExtra("position", position));
		finish();
	}

	public void delete(View view) {
		setResult(ChatActivity2.RESULT_CODE_DELETE, new Intent().putExtra("position", position));
		finish();
	}

	public void open(View v) {
		setResult(ChatActivity2.RESULT_CODE_OPEN, new Intent().putExtra("position", position));
		finish();
	}

	public void download(View v) {
		setResult(ChatActivity2.RESULT_CODE_DWONLOAD, new Intent().putExtra("position", position));
		finish();
	}

	public void toCloud(View v) {
		setResult(ChatActivity2.RESULT_CODE_TO_CLOUD, new Intent().putExtra("position", position));
		finish();
	}
